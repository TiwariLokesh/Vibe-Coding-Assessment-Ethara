...
class EmployeeService(BaseService):
    ...
    def build_employee(self, **payload: object) -> Employee:
        ...
        if self.employee_repository.get_by_employee_code(employee_code):
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Duplicate employee code")
    ...
    def update_employee(self, employee: Employee, **payload: object) -> Employee:
        ...
        if new_code and new_code != employee.employee_code:
            existing = self.employee_repository.get_by_employee_code(str(new_code))
            if existing and existing.id != employee.id:
                raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Duplicate employee code")
        if new_email and new_email != employee.email:
            existing = self.employee_repository.get_by_email(str(new_email))
            if existing and existing.id != employee.id:
                raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Duplicate email")
        if new_project_id is not None and self.project_repository.get_by_id(int(new_project_id)) is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Project not found")
